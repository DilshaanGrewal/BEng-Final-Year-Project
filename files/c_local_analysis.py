...
if "quantized" in load_model_name:
...
if "quantized" not in load_model_name:
    prototype_max_connection = torch.argmax(ppnet.last_layer.weight, dim=0)
else:
    prototype_max_connection = torch.argmax(ppnet.dequant(ppnet.last_layer.weight()), dim=0)
...
img_variable = Variable(img_tensor.unsqueeze(0)).float() /255

images_test = img_variable.to(device)
labels_test = torch.tensor([test_image_label])

if "quantized" in load_model_name:
    ll = ppnet.dequant(ppnet.last_layer.weight())
else:
    ll = ppnet.last_layer.weight
...
for i in range(1,6):
    ...
    if "quantized" not in load_model_name:
        ...
        f.write('last layer connection with predicted class: {0} \n'.format(ppnet.dequant(ppnet.last_layer.weight())[predicted_cls][sorted_indices_act[-i].item()]))
    f.write('proto index:')
    ...
    for class_id_ in range(num_classes):
        ...
        if "quantized" not in load_model_name:
            ...
            f.write(str(ppnet.dequant(ppnet.last_layer.weight())[class_id_][sorted_indices_act[-i].item()]) + '\n')
    f.close()
    if "quantized" not in load_model_name:
        log('last layer connection with predicted class: {0}'.format(ppnet.last_layer.weight[predicted_cls][sorted_indices_act[-i].item()]))
    else:
        log('last layer connection with predicted class: {0}'.format(ppnet.dequant(ppnet.last_layer.weight())[predicted_cls][sorted_indices_act[-i].item()]))
...
for i,c in enumerate(topk_classes.detach().cpu().numpy()):
    ...
    for j in reversed(sorted_indices_cls_act.detach().cpu().numpy()):
        ...
        if "quantized" not in load_model_name:
            ...
            log('last layer connection: {0}'.format(ppnet.dequant(ppnet.last_layer.weight())[c][prototype_index]))
        ...
        if "quantized" not in load_model_name:
            ...
            f.write('last layer connection: {0:.3f}\n'.format(ppnet.dequant(ppnet.last_layer.weight())[c][prototype_index]))
        ...
        for class_id_ in range(num_classes):
            ...
            if "quantized" not in load_model_name:
                ...
                f.write(str(ppnet.dequant(ppnet.last_layer.weight())[class_id_][prototype_index]) + '\n')
        f.close()
        # show the most highly activated patch of the image by this prototype
        high_act_patch_indices = find_high_activation_crop(upsampled_activation_pattern)
        high_act_patch = original_img[high_act_patch_indices[0]:high_act_patch_indices[1],
...
file_name = '/home/times1.txt' if 'quantized' in load_model_name else '/home/times2.txt'
times = open(file_name, 'a')
