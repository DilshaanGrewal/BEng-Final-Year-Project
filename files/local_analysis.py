...
prototype_max_connection = torch.argmax(ll, dim=0)
...
for i in range(1,6):
    ...
    f.write('last layer connection with predicted class: {0} \n'.format(ll[predicted_cls][sorted_indices_act[-i].item()]))
    f.write('proto index:')
    f.write(str(sorted_indices_act[-i].item()) + '\n')
    for class_id_ in range(num_classes):
        f.write(f'proto connection to class {class_id_}:')
        f.write(str(ll[class_id_][sorted_indices_act[-i].item()]) + '\n')
    f.close()
    log('last layer connection with predicted class: {0}'.format(ll[predicted_cls][sorted_indices_act[-i].item()]))
...
for i,c in enumerate(topk_classes.detach().cpu().numpy()):
    ...
    for j in reversed(sorted_indices_cls_act.detach().cpu().numpy()):
        ...
        log('last layer connection: {0}'.format(ll[c][prototype_index]))
